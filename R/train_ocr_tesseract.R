...
file_name <- "img ENG/Steinmetz Russia J-Term Journal 1978.pdf"
...
pngfiles <- pdftools::pdf_convert(
  file_name,
  dpi = 600,
  filenames = paste0("img ENG/png/", 1:info$pages, ".png")
)
...
process_page <- function(file_name) {
  ...
  page_img <- image_modulate(page_img, brightness = 90, saturation = 50)
  page_img
  page_img <- image_contrast(page_img, sharpen = 100)
  page_img
  ...
  de_page <- de_page %>%
    ...
    mutate(
      German = ifelse(
        str_detect(German, "[a-zA-Z]3"),
        str_replace(German, "3", "s"),
        German
      )
    ) %>%
  ...
  words_df2 <- words_df %>%
    ...
    mutate(
      word_alt = ifelse(str_to_title(word) == suggestion, suggestion, word_alt)
    ) %>%
    {
      .
    }
  ...
  en_page <- unlist(cur_page, recursive = FALSE) %>%
    enframe(name = NULL, value = "English") %>%
...
NS_Church_History %>%
  unnest(content) %>%
  write_csv("data/jb_church_history.csv")
